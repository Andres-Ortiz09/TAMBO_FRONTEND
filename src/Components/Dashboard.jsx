...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    setProductosBajoStock(productos.filter(p => Number(p.stock) <= 10));
  }, []);
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '2rem 0' }}>
      ...
      <div style={gridStyle}>
        ...
        <div style={{ ...cardStyle, background: '#eebbc3', color: '#232946' }}>
          <span style={{ fontSize: 18 }}>Bajo Stock</span>
          <span style={{ fontSize: 36, fontWeight: 700 }}>{productosBajoStock.length}</span>
        </div>
      </div>
                ...
                <tbody>
                  {productosBajoStock.map((p, idx) => (
                    <tr key={idx} style={{ borderBottom: '1px solid #eee' }}>
                      ...
                      <td style={{ padding: 8, textAlign: 'center', color: '#d7263d', fontWeight: 700 }}>{p.stock}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
